...
class ImageRenamer:
    ...
    def _build_ui(self):
        ...
        digits_spin = ttk.Spinbox(name_frame, from_=1, to=6, width=3, textvariable=self.digits_var, font=("", 14))
        digits_spin.pack(side=tk.LEFT, padx=(5, 10))

        ttk.Button(name_frame, text="リネーム実行", style="Big.TButton", command=self._execute_rename).pack(side=tk.LEFT)

        # --- ドロップエリア / ファイル選択ボタン ---
        drop_frame = ttk.LabelFrame(self.root, text="画像ファイル（ドラッグ＆ドロップまたはボタンで追加）", padding=10)
        ...
        ttk.Checkbutton(opt_frame, text="作成日時を表示", variable=self.show_created_var,
                        command=self._toggle_created).pack(side=tk.RIGHT)
        ...
        ttk.Button(btn_frame, text="ファイル追加", style="Big.TButton", command=self._add_files).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="クリア", style="Big.TButton", command=self._clear_files).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="プレビュー更新", style="Big.TButton", command=self._update_preview).pack(side=tk.LEFT, padx=5)

        self.topmost_btn = ttk.Button(btn_frame, text="最前面: OFF", style="Big.TButton", command=self._toggle_topmost)
        self.topmost_btn.pack(side=tk.RIGHT, padx=5)

        # --- ステータスバー ---
        self.status_var = tk.StringVar(value="画像ファイルをドラッグ＆ドロップしてください" if HAS_DND else "「ファイル追加」ボタンから画像を選択してください")
        ttk.Label(self.root, textvariable=self.status_var, relief=tk.SUNKEN, padding=5, font=("", 11)).pack(fill=tk.X, side=tk.BOTTOM)
    ...
    def _add_files(self):
        paths = filedialog.askopenfilenames(
            title="画像ファイルを選択",
            filetypes=[("画像ファイル", " ".join(f"*{ext}" for ext in IMAGE_EXTENSIONS)), ("すべて", "*.*")]
        )
        if paths:
            self._add_paths(paths)
    ...
    def _get_new_names(self):
        ...
        for i, (filepath, _) in enumerate(self.files, start=1):
            ...
            new_name = f"{base_name}_{str(i).zfill(digits)}{ext}"
    ...
    def _update_preview(self):
        ...
        for i, ((filepath, ctime), new_name) in enumerate(zip(self.files, new_names), start=1):
    ...
    def _execute_rename(self):
        ...
        # 確認ダイアログ
        if not messagebox.askyesno("確認", f"{len(self.files)}件のファイルをリネームしますか？"):
            return
        ...
        try:
            for i, (filepath, _) in enumerate(self.files):
                ...
                temp_name = os.path.join(dirpath, f"__temp_rename_{i}_{os.path.basename(filepath)}")
            ...
            for temp_path, final_path in temp_map:
                if os.path.exists(final_path):
                    messagebox.showerror("エラー", f"ファイルが既に存在します: {os.path.basename(final_path)}\nリネームを中断します。")
                    # ロールバック
                    for t, _ in temp_map:
                        if os.path.exists(t):
        ...
        if len(new_names) == 1:
            ...
            self.status_var.set(f"リネーム完了: {first} ... {last}（{len(self.files)}件）")
        self.files.clear()
        self.tree.delete(*self.tree.get_children())
